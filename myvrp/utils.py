...
def text2json(customize=False,num_range=101):
    '''gavrptw.uitls.text2json(customize=False)'''
    text_data_dir = os.path.join(
        BASE_DIR, 'data', 'text_customize' if customize else 'text')
    json_data_dir = os.path.join(
        BASE_DIR, 'data', 'json_customize' if customize else 'json')
    for text_file in map(lambda text_filename: os.path.join(text_data_dir, text_filename),
        ...
        customers = ['depart'] + [f'customer_{x}' for x in range(1, num_range)]
